...
export default function DashboardPage() {
  ...
  /* ── Formatters: never return "—", always show $0.00 or 0 ── */
  const fmt = (n: number | undefined | null) =>
    n === undefined || n === null ? "0" : n.toLocaleString("en-US");
  ...
  const fmtRpm = (n: number | undefined | null) =>
    n === undefined || n === null ? "0.00" : n.toFixed(2);

  /* ── Driver coverage gauge ── */
  const driverCoverage = kpis
    ? kpis.active_drivers > 0
      ? Math.round((kpis.on_trip_drivers / kpis.active_drivers) * 1000) / 10
      : 0
    : 0;
  const gaugeData = [
  ...
  /* ── Revenue progress bar: $100k = 100% ── */
  const grossRevenuePct = kpis?.gross_revenue
    ? Math.min((kpis.gross_revenue / 100_000) * 100, 100)
    : 0;
  ...
  if (kpis) {
    if (kpis.active_loads > 0)
      tasks.push({ text: `Mark ${kpis.active_loads} active loads as delivered`, href: "/loads", color: "var(--primary)" });
    if (kpis.offer_loads > 0)
      tasks.push({ text: `Dispatch ${kpis.offer_loads} offer loads`, href: "/loads", color: "var(--warning)" });
    if (kpis.active_drivers > 0 && kpis.on_trip_drivers < kpis.active_drivers)
      tasks.push({ text: `Assign loads to ${kpis.active_drivers - kpis.on_trip_drivers} available drivers`, href: "/drivers", color: "var(--success)" });
  }
  if (fleetAvailable > 0)
    tasks.push({ text: `${fleetAvailable} trucks available for dispatch`, href: "/fleet", color: "var(--info)" });
    ...
    <div className="page-container" style={{ overflow: "auto" }}>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="card-section">
          ...
          {fleetTotal > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <span className="body-md font-medium text-[var(--on-surface-variant)]">No fleet registered yet</span>
              <button
                onClick={() => router.push("/fleet/new")}
                className="btn btn-sm btn-primary"
              >
                <Plus className="h-3 w-3" /> Register First Truck
              </button>
            </div>
          )}
        </div>
        ...
        <div className="card-section card-section--green">
          ...
          {tasks.length === 0 ? (
            // Differentiate: no data at all vs. genuinely all caught up
            !kpis || (kpis.active_loads === 0 && kpis.offer_loads === 0) ? (
              <div className="flex flex-col items-center justify-center py-8 gap-3">
                <Package className="w-8 h-8" style={{ color: "var(--primary)", opacity: 0.35 }} />
                <span className="title-sm text-[var(--on-surface)] text-center">Ready to dispatch?</span>
                <span className="body-sm text-center text-[var(--on-surface-variant)]">Create your first load to get started</span>
                <button
              ...
              {[
                ...
                <div key={s.name}>
                  ...
                  <div className="progress-bar-track">
                    <div className="progress-bar-fill" style={{ width: `${Math.max(s.pct, s.qty > 0 ? 2 : 0)}%`, background: s.color }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {fleetTotal > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <button
                onClick={() => router.push("/fleet/new")}
                className="btn btn-sm"
                style={{ background: "var(--accent-cyan)", color: "#fff", border: "none" }}
              >
        ...
        <div className="card-section card-section--green">
            ...
            {[
              {
                label: "Gross Revenue",
                value: fmtCurrency(kpis?.gross_revenue),
                width: `${grossRevenuePct}%`,
                color: "var(--success)",
              },
              {
                label: "Avg Rate Per Mile",
                value: `$${fmtRpm(kpis?.avg_rpm)}`,
                width: `${Math.min((kpis?.avg_rpm ?? 0) * 30, 100)}%`,
                color: "var(--primary)",
              },
              {
                label: "Fleet Utilization",
                value: `${fleet?.utilization_rate ?? 0}%`,
                width: `${fleet?.utilization_rate ?? 0}%`,
                ...
                <div className="progress-bar-track">
                  <div
                    ...
                    style={{
                      ...
                      minWidth: item.width === "0%" || item.width === "0" ? 0 : undefined,
                    }}
                  />
                </div>
              </div>
          ...
          <p className="text-[10px] mt-4" style={{ color: "var(--on-surface-variant)", opacity: 0.6 }}>
            Revenue bar scales to $100k target. RPM × 30 = bar width.
          </p>
        </div>
      </div>
    </div>
